    ...
    const saveTransaction = async () => {
        ...
        try {
            await runTransaction(db, async (transaction) => {
                ...
                // 🟢 1. อัปเดตการบันทึกข้อมูลบิล: เพิ่ม memberName และ memberPhone
                const billData = {
                    ...
                    note: cartNote, 
                    memberName: currentMember?.name || 'ลูกค้าทั่วไป', // 👈 บันทึกชื่อ
                    memberPhone: currentMember?.phone || memberPhone || '', // 👈 บันทึกเบอร์
                    createdAt: new Date()
                };
        ...
        <>
            ...
            <div className="p-4 md:p-8 h-full max-w-[1600px] mx-auto overflow-hidden print:hidden">
                <div className="flex flex-col lg:flex-row gap-8 h-full items-start pb-28">
                        ...
                        {!isPaymentStep ? (
                            <>
                                    ...
                                    {selectedProduct && (
                                        <div className="space-y-4 animate-in fade-in zoom-in-95">
                                            <div className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border">
                                                <span className="text-[10px] font-black text-slate-400 uppercase">คงเหลือ: {products.find(p=>p.id===selectedProduct)?.stock}</span>
                                            </div>
                                            <div className="grid grid-cols-2 gap-4">
                                                ...
                                                <Button onClick={addToCart} className="col-span-full py-4 font-black">เพิ่มลงตะกร้า</Button>
                                            </div>
                                        </div>
                                    )}
                                </Card>
                                ...
                                <Card className="!p-5 border-2 border-dashed border-slate-200 bg-white">
                                    <div className="flex items-center gap-2 mb-3 text-slate-400 text-[10px] font-black uppercase"><MessageSquare size={16}/> บันทึกหมายเหตุบิล</div>
                                    <textarea className="w-full bg-slate-50 rounded-xl p-4 text-sm min-h-[80px] outline-none focus:ring-4 focus:ring-blue-50 transition-all" placeholder="รายละเอียด..." value={cartNote} onChange={e => setCartNote(e.target.value)} />
                                </Card>
                            </>
                        ) : (
                            <Card className="!p-8 border-2 border-blue-100 shadow-xl animate-in zoom-in-95 text-center relative overflow-hidden">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="lg:hidden pt-4 pb-2">
                                        <Button 
                                            className="w-full py-5 text-xl font-black shadow-xl shadow-blue-200" 
                                            onClick={saveTransaction} 
                                            disabled={receivedAmount < finalAmount}
                                        >
                                            {receivedAmount < finalAmount ? 'เงินไม่พอ' : 'ยืนยันและจบบิล'}
                                        </Button>
                                    </div>
                                </div>
                            </Card>
                    ...
                    <aside className="hidden lg:flex w-[40%] bg-white border border-slate-100 rounded-[2.5rem] flex-col h-[calc(100vh-140px)] sticky top-8 shadow-sm overflow-hidden">
                        ...
                        <div className="p-8 bg-slate-50 border-t space-y-4">
                            <div className="flex justify-between items-center"><span className="text-slate-400 font-black uppercase text-xs tracking-widest">ยอดรวมทั้งสิ้น</span><span className="text-4xl font-black text-blue-600 italic">฿{finalAmount.toLocaleString()}</span></div>
                            <div className="flex gap-3">
                                {!isPaymentStep && <Button variant="secondary" className="flex-1 py-5 font-black bg-white shadow-sm" onClick={holdCurrentBill} disabled={cart.length === 0}><PauseCircle size={20} /> พักบิล</Button>}
                                <Button className={`py-5 text-xl font-black shadow-xl shadow-blue-100 ${isPaymentStep ? 'w-full' : 'flex-[2]'}`} onClick={() => { if(!isPaymentStep) setIsPaymentStep(true); else saveTransaction(); }} disabled={cart.length === 0 || (isPaymentStep && receivedAmount < finalAmount)}>
                ...
                {!isPaymentStep && cart.length > 0 && (
                    <div className="lg:hidden fixed bottom-24 left-4 right-4 z-[50] animate-in fade-in slide-in-from-bottom-10 print:hidden">
                        <button onClick={() => setShowMobileCart(true)} className="w-full bg-slate-900 text-white p-5 rounded-[2rem] shadow-2xl flex justify-between items-center border border-slate-700 active:scale-95 transition-all">
                            <div className="flex items-center gap-3">
                                <div className="bg-blue-600 p-2 rounded-2xl text-white"><ShoppingCart size={22} /></div>
                                <div className="text-left"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">ตะกร้า ({cart.length})</p><p className="text-lg font-black tracking-tight">ตรวจสอบรายการ</p></div>
                            </div>
                ...
                {showMobileCart && (
                    <div className="lg:hidden fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[150] flex flex-col print:hidden">
                        <div className="flex-1 mt-16 bg-white rounded-t-[3.5rem] flex flex-col overflow-hidden animate-in slide-in-from-bottom-full duration-500">
                            <div className="p-8 border-b flex justify-between items-center bg-slate-50/50">
                                <h3 className="font-black text-2xl text-slate-800 italic uppercase">Cart Summary</h3>
                                <button onClick={() => setShowMobileCart(false)} className="p-3 bg-white shadow-sm rounded-full text-slate-400"><X /></button>
                            </div>
                            <div className="flex-1 overflow-y-auto p-8 space-y-6">
                                {cart.map((item, idx) => (
                                    <div key={idx} className="flex justify-between items-center bg-slate-50 p-5 rounded-[2rem] border border-slate-100">
                                        <div className="flex items-center gap-4">
                                            <button onClick={() => setCart(cart.filter((_, i) => i !== idx))} className="text-red-400 p-2"><Trash2 size={22}/></button>
                                            <div><p className="font-black text-slate-800 text-base">{item.name}</p><p className="text-xs font-bold text-slate-400">{item.qty} {item.unit} x ฿{item.price.toLocaleString()}</p></div>
                                        </div>
                                        <p className="font-black text-xl text-slate-800">฿{item.total.toLocaleString()}</p>
                                    </div>
                                ))}
                                <div className="mt-4 p-5 border-2 border-dashed border-slate-100 rounded-[2rem] bg-slate-50/30">
                                    <div className="flex items-center gap-2 mb-3 text-slate-400 text-[10px] font-black uppercase tracking-widest"><MessageSquare size={16}/> บันทึกหมายเหตุ</div>
                                    <textarea className="w-full bg-white border border-slate-100 rounded-2xl p-4 text-sm min-h-[100px] outline-none shadow-sm" placeholder="รายละเอียดเพิ่มเติม..." value={cartNote} onChange={e => setCartNote(e.target.value)} />
                                </div>
                            </div>
                            <div className="p-8 bg-white border-t-2 border-slate-50 space-y-4 shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
                                <div className="flex justify-between items-center px-2"><span className="text-slate-400 font-black uppercase text-xs tracking-widest">ยอดสุทธิ</span><span className="text-4xl font-black text-blue-600 italic">฿{finalAmount.toLocaleString()}</span></div>
                                <div className="flex gap-3">
                                    <Button variant="secondary" className="flex-1 py-5 font-black bg-slate-100 shadow-sm" onClick={holdCurrentBill}><PauseCircle size={20} /> พักบิล</Button>
                ...
                {showHeldBills && (
                    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[120] flex items-center justify-center p-4 print:hidden">
                ...
                {showReceipt && lastBill && (
                    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[200] flex items-center justify-center p-4 print:hidden">
                        <Card className="max-w-md w-full p-8 text-center animate-in zoom-in-95 shadow-2xl relative">
                            ...
                            <h2 className="text-3xl font-black text-slate-800 mb-2 italic tracking-tighter uppercase">SUCCESSFUL!</h2>
                            <p className="text-slate-400 text-sm mb-6 font-bold tracking-widest uppercase">Bill No: {lastBill.docNo}</p>
                            <div className="space-y-3">
                                <Button className="w-full py-5 text-lg font-black flex items-center justify-center gap-3 shadow-xl shadow-blue-100" onClick={() => window.print()}><Printer size={24} /> พิมพ์ใบเสร็จ</Button>
