...
import { Plus, Search, QrCode, ChevronRight, Save, Wand2, Printer, Trash2 } from 'lucide-react';
...
export default function ProductView({ products, setProducts, viewState, setViewState, calculateStock, handleScanQR, handleDeleteProduct }) {
    ...
    // ฟังก์ชันช่วยลบสินค้าและกลับหน้าหลัก
        ...
        return (
            <div className="p-4 md:p-8 animate-in fade-in slide-in-from-bottom-4 duration-300 max-w-2xl mx-auto">
                    ...
                    <div id="printable-area">
                        {Array.from({ length: printQty }).map((_, i) => (
                            <div key={i} className="print-card">
                                ...
                                {tempProduct.sellPrice > 0 && <p style={{ fontSize: '12px', fontWeight: 'bold', color: 'black' }}>฿{Number(tempProduct.sellPrice).toLocaleString()}</p>}
                            </div>
                        ))}
                    </div>
                )}
                ...
                <div className="flex items-center justify-between mb-6 no-print">
                    ...
                    {/* [เพิ่มใหม่] ปุ่มลบในหน้าแก้ไข */}
                    {tempProduct.id && (
                        <button 
                            onClick={() => onConfirmDelete(tempProduct.id)}
                ...
                <Card className="no-print">
                        ...
                        <div className="col-span-full">
                            <div className="flex items-end gap-2">
                                <div className="flex-1">
                                    <Input label="รหัสสินค้า (SKU)" value={tempProduct.code || ''} onChange={e => setTempProduct({...tempProduct, code: e.target.value})} icon={QrCode} onIconClick={() => handleScanQR((code) => setTempProduct({...tempProduct, code}))} placeholder="สแกน หรือ กดปุ่มสุ่มรหัส ->" />
                                </div>
                                <button type="button" onClick={handleGenerateRandomCode} className="mb-4 p-3.5 bg-purple-50 text-purple-600 rounded-xl border border-purple-100 hover:bg-purple-100 shadow-sm active:scale-95"><Wand2 size={20} /></button>
                            </div>

                            {tempProduct.code && (
                                <div className="bg-blue-50 border border-blue-100 rounded-2xl p-6 mb-6 mt-2">
                                    <div className="flex flex-col items-center gap-4">
                                        <div className="bg-white p-4 rounded-xl shadow-md"><QRCode value={tempProduct.code} size={100} style={{ height: "auto", maxWidth: "100%" }} /></div>
                                        <div className="grid grid-cols-2 gap-4 w-full">
                                            <div>
                                                <label className="text-xs font-bold text-blue-600 mb-1 block">ขนาด QR (px)</label>
                                                <input type="number" value={printSize} onChange={(e) => setPrintSize(e.target.value)} className="w-full p-2.5 bg-white border border-blue-200 rounded-lg text-center font-bold" />
                                            </div>
                                            <div>
                                                <label className="text-xs font-bold text-blue-600 mb-1 block">จำนวนที่จะพิมพ์</label>
                                                <input type="number" value={printQty} onChange={(e) => setPrintQty(Math.max(1, parseInt(e.target.value) || 1))} className="w-full p-2.5 bg-white border border-blue-200 rounded-lg text-center font-bold" />
                                            </div>
                                        </div>
                                        <button type="button" onClick={handlePrintQR} className="flex items-center gap-2 px-8 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 shadow-lg active:scale-95 w-full justify-center"><Printer size={20} /> พิมพ์ฉลาก {printQty > 1 ? `(${printQty} ใบ)` : ''}</button>
                                    </div>
                                </div>
                            )}
                        </div>

                        <div className="col-span-full"><Input label="ชื่อสินค้า" value={tempProduct.name || ''} onChange={e => setTempProduct({...tempProduct, name: e.target.value})} placeholder="ระบุชื่อสินค้า..." /></div>
                    ...
                    <Button onClick={handleSaveProduct} className="w-full mt-6 py-4 text-base no-print"><Save size={20} /> บันทึกข้อมูลสินค้า</Button>
                </Card>
            </div>
        );
    }
    ...
    return (
        <div className="p-4 md:p-8 space-y-6 max-w-4xl mx-auto pb-24">
            ...
            <div className="relative shadow-sm rounded-xl no-print">
                <input type="text" placeholder="ค้นหาชื่อ หรือ รหัสสินค้า..." className="w-full bg-white border border-gray-200 rounded-xl py-3.5 pl-12 pr-4 text-sm focus:ring-2 focus:ring-blue-100 transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 no-print">
                {products.filter(p => p.name?.toLowerCase().includes(searchTerm.toLowerCase()) || p.code?.includes(searchTerm)).map(p => {
                    ...
                    return (
                        <div key={p.id} className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4 hover:border-blue-300 transition-all group relative">
                            {/* ส่วนคลิกเพื่อเข้าไปแก้ไขสินค้า */}
                            <div className="flex-1 flex items-center gap-4 cursor-pointer" onClick={() => { setTempProduct(p); setViewState('form'); }}>
                            ...
                            {/* [เพิ่มใหม่] ปุ่มลบด่วนที่การ์ดสินค้า */}
                            <button 
